...
### Layout model
- **Navbar** — always visible. Contains (left→right): hamburger (`#btn-drawer`) toggles the file drawer; filename display; **font picker** (`#font-picker-wrap` → `#font-picker-btn`); **font size picker** (`#size-picker-wrap` → `#size-picker-btn`, options 13/15/16/18/20px); **Markdown split button** (`#btn-split`); **dark/light mode toggle** (`#btn-theme`, moon/sun icon); divider; save button. Font picker, size picker, and Markdown button share the same `height: 36px` so they align visually.
...
**Editor font.** The editor font is controlled by the `--f-editor` CSS custom property on `:root`. The navbar font picker (`#font-picker-btn`) calls `toggleNavPicker('font')` to open a `.nav-picker-dropdown`; selecting an option calls `selectFont(el)` which calls `setFont(el.dataset.font)` and updates the button label. Available fonts: JetBrains Mono (default), Inter, Merriweather. Do not hardcode `font-family` on `.ql-editor` — always use `var(--f-editor)`.
...
**Font size.** Controlled by `--font-size-editor` CSS custom property (default `16px`). `.ql-editor` uses `font-size: var(--font-size-editor)`. The navbar size picker (`#size-picker-btn`) calls `toggleNavPicker('size')` to open its `.nav-picker-dropdown`; selecting calls `selectSize(el)` which calls `setFontSize(el.dataset.size)`. Do not hardcode `font-size` on `.ql-editor`.

**Default editor font is JetBrains Mono.** `--f-editor` is set to `'JetBrains Mono', monospace` in `:root`. The font picker dropdown has the `selected` class on the JetBrains Mono option and `#font-picker-label` reads "JetBrains Mono". Both must be kept in sync if the default changes.
...
**Custom nav pickers (font & size).** `#font-picker-wrap` and `#size-picker-wrap` are `.nav-picker-wrap` containers holding a `.nav-picker` button and a `.nav-picker-dropdown` panel. `toggleNavPicker(which)` opens/closes the correct dropdown and closes the other. `selectFont(el)` / `selectSize(el)` call `setFont` / `setFontSize`, update the label span, mark the selected option, and close the dropdown. A delegated `document click` handler closes all `.nav-picker-dropdown` elements when clicking outside `.nav-picker-wrap`. Mobile hides both wrappers via `#font-picker-wrap, #size-picker-wrap { display: none }` in the `≤767px` media query.

**Section collapse overlay.** A `#collapse-overlay` div is injected inside `.ql-container` (outside `.ql-editor`, safe from Quill's normalizer). `updateCollapseOverlay()` rebuilds it on every scroll and content change: it calls `buildSections()` to pair heading elements with their following content, then places a `.collapse-btn` for each visible heading. The button's `top` is set to the heading's vertical centre relative to the overlay; `left` is computed dynamically as `heading.left - overlay.left - 24px` so the button sits just to the left of the heading text regardless of editor padding or screen width. The overlay is `pointer-events: none` except on the buttons themselves. `collapsedHeadings` is a `WeakSet`; `toggleCollapse(el)` adds/removes entries and calls `applyCollapseState()` + `scheduleTreeSync()`.

**Markdown pane scroll sync.** `syncMdPaneScroll()` mirrors the Quill editor scroll position to `#md-textarea` proportionally: `ratio = scrollTop / (scrollHeight - clientHeight)`. It is called via `requestAnimationFrame` inside `quill.root`'s scroll listener (only when `S.splitOpen`). `scrollMdPaneToHeading(text)` is called 350 ms after `scrollToHeading()` to precisely land the markdown textarea on the matching heading line by scanning raw markdown line-by-line.

**Word count.** `updateWordCount()` reads `quill.root.innerText`, splits on whitespace, and writes `N w` (or `—` for empty) to `#md-word-count` in `.md-pane-header`. Called at the end of `syncMdPane()` and `switchTab()` (when split is open). The `.md-pane-header` uses `justify-content: flex-start` + `margin-left: auto` on `.md-word-count` to push the count to the right before the Copy button.

**`==highlight==` mark blot.** A `MarkBlot` (inline, `<mark>` tag) is registered before `initQuill()`. A Marked extension converts `==text==` to `<mark>text</mark>` on import. A Turndown rule converts `<mark>` back to `==text==` on export. The toolbar has a custom `.ql-mark` button that toggles `quill.format('mark', ...)`. Light mode: solid `#ffe066` background, `#1a1a1a` text. Dark mode: `#7a6000` background, `#ffe8a0` text. Do not add a second `.ql-editor mark` rule later in the stylesheet — it will override the first and the highlight will become invisible.
...
- Light theme colours are tuned for WCAG contrast against `#f5f3ee` (warm cream base). If changing `--c-accent` in light mode, re-verify contrast on both `--c-surface` (`#ffffff`) and `--c-base` backgrounds.
